...
import { BaseRenderable, createCliRenderer, RootRenderable, TextRenderable } from '@opentui/core';
import { treeToString, Logger } from './common/logger';
 ...
 */
const INTERNAL_OPENTUI_PLATFORM_PROVIDERS: StaticProvider[] = [{ provide: Logger, useValue: Logger.instance }];
...
export async function bootstrapApplication(
  ...
  applicationRef = await ɵinternalCreateApplication({
  ...
  Logger.instance.log('after bootstrap');
  dumpState(applicationRef.injector.get(TUI_ROOT_NODE).getRoot()!);

  setTimeout(() => {
    Logger.instance.log('1 second after bootstrap');
    dumpState(applicationRef.injector.get(TUI_ROOT_NODE).getRoot()!);
  }, 2000);
  return applicationRef;
}

function dumpState(rootNode: TuiNode<RootRenderable>) {
  Logger.instance.log(
    `OpenTUI tree`,
    treeToString<BaseRenderable>(
      rootNode.renderable()!,
      (n) => n.getChildren(),
      (n) => {
        let str = `${n.constructor.name}#${n.id}`;
        if (n instanceof TextRenderable) {
          str += ` "${n.content?.chunks.map((c) => c.text).join()}"`;
        }
        return str;
      },
    ).join('\n'),
  );
  Logger.instance.log(`TUI node tree`, treeToString<TuiNode>(rootNode, (c) => c.children()).join('\n'));
}
